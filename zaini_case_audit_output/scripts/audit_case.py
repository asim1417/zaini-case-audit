...
audit_case.py
====================
نظام فحص آلي ومنهجي لمستندات القضايا القانونية.
...
# (إن لم يُحدَّد، يكون السلوك الافتراضي = مجلد المخرجات الأب).
...
# ---------------------------------------------------------------------------
CASE_NUMBERS_KNOWN = []   # تُملأ من case_config.json (مفتاح case.number / case_numbers)
DEED_NUMBERS_KNOWN = []   # تُملأ من case_config.json (deed_numbers_known)
PARTIES = {}              # {"الاسم الظاهر": ["أنماط المطابقة"]} — من case_config.json (parties)
OTHER_ACTORS = {}         # من case_config.json (other_actors)
COMPANY_PATTERNS = []     # من case_config.json (company_patterns)

# ---------------------------------------------------------------------------
# تحميل إعداد القضية من ملف خارجي (case_config.json) — لجعل النظام عاماً لأي قضية
# إن وُجد الملف، يَستبدل الثوابت أعلاه. وإلا تبقى القوائم فارغة (تُكتشف الأطراف آلياً من النص).
# يُبحث عنه في: متغيّر البيئة CASE_CONFIG، أو جذر المخرجات، أو staging/.
# ---------------------------------------------------------------------------
CASE_TITLE = ""        # من case_config.json (case.title)
CASE_FOLDER_NAME = ""  # من case_config.json (case.case_folder_name)
...
def extract_entities(raw_text):
    ...
    if find_terms(hay, COMPANY_PATTERNS):
        ents["company"].append(COMPANY_PATTERNS[0] if COMPANY_PATTERNS else "الشركة محل النزاع")
...
# ---------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="فحص آلي لمستندات القضايا القانونية")
    ...
    md = []
    md.append("# تقرير الفحص الآلي الأولي لمستندات القضية")
    md.append("")
