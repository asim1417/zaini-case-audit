...
هذه الوحدة (`service/cloud_drives.py`) تربط محرّك الفحص القانوني بمساحات التخزين السحابية،
...
## هل يلزم مفاتيح ذكاء اصطناعي؟

**لا.** القراءة الأساسية تعمل دائماً بلا أي مفاتيح: محرّك Tesseract العربي محلي ومجاني
(مع تعزيزات المشروع)، وكذلك التصنيف واستخراج الكيانات والتقارير — كلها قواعد محلية.
المفاتيح كلها **اختيارية**:

| المفتاح | ماذا يضيف؟ | التكلفة |
|---|---|---|
| بيانات Google / OneDrive | الجلب المباشر من السحابة بدل الرفع اليدوي | مجاني |
| Azure Document Intelligence | OCR سحابي أدق للمسح الضوئي الصعب/الباهت (يتراجع للمحلي تلقائياً عند أي خطأ) | مدفوع لكل صفحة (شريحة مجانية شهرية) |

## التفعيل من الواجهة (بلا لمس الخادم)

افتح واجهة الخدمة `GET /` → **«⚙ التفعيل والإعدادات»**: ألصق مفاتيح Azure OCR أو
حساب خدمة Google أو تطبيق OneDrive واحفظ — تُخزَّن في `settings.json` داخل حجم
البيانات (تبقى بعد إعادة التشغيل) **وتسري فوراً** على المهام التالية دون إعادة تشغيل.
برمجياً: `GET/POST /admin/settings`. الحقل الفارغ لا يغيّر المحفوظ، والقيمة `-` تحذفه.

> ⚠ احمِ هذه اللوحة بضبط `API_KEY` على الخدمة؛ وإلا فأي من يصل للرابط يستطيع تعديل الإعدادات.

يمكن أيضاً إرفاق **بيانات القضية** (رقمها وأسماء أطرافها وأرقام الصكوك) مع أي مهمّة —
من الواجهة أو من نافذة «☁ سحابة» في العارض — فتُبنى `case_config.json` تلقائياً
وتتحسن التقارير (ربط الأطراف والأرقام المعروفة).
